...
Root directory: /var/www/$APP_USER/backend-dev
...
pm2 --name $APP_USER-backend start yarn -- run start --port 3002
...
pm2 --name $APP_USER-admin start yarn -- run start --port 3003
