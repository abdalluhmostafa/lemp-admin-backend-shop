...
## app Nginx config
cat > /etc/nginx/sites-available/app.$DOMAIN_NAME <<EOF

server {
        index index.html index.htm;
        server_name app.$DOMAIN_NAME;
        access_log /var/log/nginx/app-$DOMAIN_NAME-access.log;
        error_log /var/log/nginx/app-$DOMAIN_NAME-error.log;
location / {
...
ln -s /etc/nginx/sites-available/app.$DOMAIN_NAME /etc/nginx/sites-enabled/
